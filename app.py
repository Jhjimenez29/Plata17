...
# =============================================================
    ...
    if df_historial is None or df_historial.empty:
        ...
        # Detección dinámica de columnas para evitar fallos de claves
        cols_df = df_historial.columns.tolist()
        col_cliente = "Nombre Cliente" if "Nombre Cliente" in cols_df else cols_df[0]
        col_fecha = "Fecha" if "Fecha" in cols_df else (cols_df[1] if len(cols_df) > 1 else cols_df[0])

        with col_f1:
            opts_cli = [opcion_default] + sorted(df_historial[col_cliente].dropna().astype(str).unique().tolist())
            cliente_sel = st.selectbox("Filtrar por Cliente:", options=opts_cli)
        with col_f2:
            opts_fec = [opcion_default] + sorted(df_historial[col_fecha].dropna().astype(str).unique().tolist(), reverse=True)
        ...
        if filtro_cli:
            df_filtrado = df_filtrado[df_filtrado[col_cliente].astype(str) == cliente_sel]
        if filtro_fec:
            df_filtrado = df_filtrado[df_filtrado[col_fecha].astype(str) == fecha_sel]
        ...
        if not df_filtrado.empty:
            ...
            try:
                ...
                st.warning("⚠️ No se pudo generar el archivo de descarga en Excel.")
        else:
